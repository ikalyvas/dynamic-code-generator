...
test_enc = '/scratch/kalyvas/git_trunk/test/unittest/lib/libisu/src/LibIsuEnc_' + \
    interface + '_tlvs.cpp'
test_dec = '/scratch/kalyvas/git_trunk/test/unittest/lib/libisu/src/LibIsuDec_' + \
    interface + '_tlvs.cpp'
    ...
    def bit_worker(struct_fields, payload):
        if re.search('reserved|spare|unused', var_name):
            ...
            var_value = random.randint(
                0, int(var_size)**2 - 1) if int(var_size) != 1 else random.randint(0, 1)
            # bin function spits 0bXXXXX.we only want to keep XXXXX
            bin_value = bin(var_value)[2:]
            byte_value.insert(0, bin_value)
            exp = (payload + '.' + var_name + '=' + str(var_value) + ';', '')
            struct_fields.append(exp)
...
def handle_large_hex(hex_value):
    ...
    # odd number of hex_value ...ie 0x1CB or OxFA2D2,etc (3,5,7,9,...) so we
    # add a zero in front.else leave it.
    # put away the L indicating the LARGE number
    hex_value = hex_value[2:].strip('L')
...
def fill_bitfields(byte_value):
  ...
  #  print byte_value
    hex_value = tmp_hex_value[:2] + '0' + tmp_hex_value[-1].upper() if int(
        tmp_hex_value, 16) in range(16) else tmp_hex_value[:2] + tmp_hex_value[2:].upper()
    return hex_value
...
def fill_values(i, line, struct_fields, payload, bits):
    ...
    # array that will be memset with zeroes
    elif re.search('\[(\d+)\]', line) and ('reserved' in line or 'spare' in line):
        ...
        exp = ('memset(' + payload + '.' + array_name + ',0,' +
               str(int(array_size) * (int(byte_num) / 8)) + ');', hex_value.rstrip(','))
        struct_fields.append(exp)

    # ordinary array that will be filled randomly depending on the size
    elif re.search('\[(\d+)\]', line):
        array_name = re.search('(?= )(.+)(?=\[)', line).group(1).strip()
        array_size = re.search('(?<=\[)(\d+)(?=\])', line).group(1)
        byte_num = re.search('(?<=u)(\d+)(?=\s)', line).group(1)
        for i in range(int(array_size)):
            # alternatively we can use the hex_value formula when its a
            # bitfield..see above...
            dec = random.randint(0, 2**int(byte_num) - 1)
            #hex_value = hex(dec)[:2]+hex(dec)[2:].upper()
            hex_value = handle_large_hex(hex(dec))
            exp = (payload + '.' + array_name +
                   '[' + str(i) + ']=' + str(dec) + ';', hex_value)
            struct_fields.append(exp)
        ...
        exp = (payload + '.' + var_name + '=' +
               str(var_value) + ';', hex_value)
        struct_fields.append(exp)

    # everything else that is usually unsigned (8-bit,16-bit,32-bit,64-bit)
    else:
        var_name = re.search('(?<= )(\w+)(?=;)', line).group(1).strip()
        var_size = re.search('^u(\d+)', line).group(1).strip()
...
def create_test_buffer(struct_fields, payload):
    ...
    for i, (exp, val) in enumerate(struct_fields):
        # last line to be processed that is not bitfield
        if val and i == len(struct_fields) - 1:
            val = ' ' * 22 + val + '};    //' + \
                re.search(payload + '.' + '(.+)(?=\=|,)',
                          exp).group(1).rstrip(',0')
            test_buff += '\n' + val
        elif val:
            val = ' ' * 22 + val + ',    //' + \
                re.search(payload + '.' + '(.+)(?=\=|,)',
            ...
            try:
                # next element has value,ie is not a bitfield.that means we
                # need to pop
                if struct_fields[i + 1][1]:
                    val = ' ' * 22 + byte_buff.pop(0)
                    test_buff += '\n' + val + ',   //byte for bitfields'
                # next element has bitfield...pass
                elif not struct_fields[i + 1][1]:
                    pass
...
def create_test_encoder(payload, hex_id_type, hex_id_length, test_buffer, struct_fields):
    ...
    with open(test_enc, 'a') as f:
        body = 'TEST_FIXTURE(Libisu_enc_' + interface + \
            '_tlvs, LibIsuEnc' + interface + '_tlvs_' + payload + '){'
        ...
        body += '\n\n'
        body += '\n' + '      CHECK_EQUAL(RETURN_OK,LIBISU_encode_' + \
            payload + '(hdr,tlv_buffer_size,&' + payload + '));'
        body += '\n' + '      ' + \
...
def create_test_decoder(payload, hex_id_type, hex_id_length, id_length, test_buffer, struct_fields):
    ...
    # length here counts the header length of the tlv_msg_hdr_t,8 is the
    # mandatory,4 is the TL,and the V
    length = 8 + 4 + int(id_length)
    hex_hdr_length = handle_large_hex(hex(length))
    # 4 is the 4 bytes of T and L.So we add to this the payload (V) to find
    # the total length
    total_length = 4 + int(id_length)
    ...
    with open(test_dec, 'a') as f:
        body = 'TEST_FIXTURE(Libisu_msg_dec_' + interface + \
            '_tlvs, LibIsuMsgDec' + interface + '_tlvs_' + payload + '){'
        ...
        body += ' ' + test_buffer
        body += '\n\n' + '   libisu_' + payload + '_t *' + payload + \
            ' = (libisu_' + payload + '_t *)malloc(sizeof(libisu_' + payload + '_t));'
        body += '\n' + '     CHECK_EQUAL(RETURN_OK,LIBISU_decode_' + \
            payload + '((tlv_msg_hdr_t *)encoded_buffer,' + payload + '));'
        body += '\n' + '     CHECK(!memcmp((u8 *)' + payload + \
            ', (u8 *)test_buff, sizeof(libisu_' + payload + '_t)));'
        body += '\n\n' + '   printf("DUMMY - decoded buffer: %s' + '\\' + \
            'n", libconv_dump_to_str((u8 *)' + payload + \
            ',sizeof(libisu_' + payload + '_t)));'
        body += '\n' + '   printf("DUMMY - test buffer: %s' + '\\' + \
            'n", libconv_dump_to_str((u8 *)test_buff,sizeof(libisu_' + \
            payload + '_t)));'
        body += '\n' + '   free(' + payload + ');'
        body += '\n}\n'
        f.write(body)
...
def execute():
    ...
    for i, line in enumerate(tmp):
        if 'TLV_ID' in line:
            ...
            # this is the T (number)of TLV
            id_type = re.search('---(\d+)', line[0]).group(1)
            hex_id_type = hex(int(id_type))[:2] + '0' + hex(int(id_type))[-1].upper() if int(
                id_type) in range(16) else hex(int(id_type))[:2] + hex(int(id_type))[2:].upper()
            # this is the length of the payload of the TLV
            id_length = re.search('---(\d+)', line[1]).group(1)
            hex_id_length = hex(int(id_length))[:2] + '0' + hex(int(id_length))[-1].upper() if int(
                id_length) in range(16) else hex(int(id_length))[:2] + hex(int(id_length))[2:].upper()
            # use case of lib_plain_buffer which occupies 1404 bytes
            hex_id_length = handle_large_hex(hex_id_length)

            # handle the endianess of TLV LENGTH here

            # length is so large it cannot fit in one byte only
            if not is_same_endian(interface) and len(hex_id_length.split(',')) > 1:
                hex_id_length = handle_endian(hex_id_length)
                hex_id_length += ',  //TLV LENGTH'

            # most cases will be handled in here...
            elif not is_same_endian(interface) and len(hex_id_length.split(',')) == 1:
           ...
           # if we do,fix that by asking about the number of bytes it occupies.
            ...
            # this is the payload name
            payload = re.search('libisu_(.+)---', line[1]).group(1)
   ...
   # global byte_buff
    ...
    create_test_encoder(payload, hex_id_type, hex_id_length,
                        test_buffer, struct_fields)
    create_test_decoder(payload, hex_id_type, hex_id_length,
                        id_length, test_buffer, struct_fields)
    tmp.close()
    os.remove('tmp.txt')

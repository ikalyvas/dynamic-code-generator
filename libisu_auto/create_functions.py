...
def append(file_ch,function,flag=""):
    ...
    if flag == 1:#we are encoding
        prototype='return_t LIBISU_encode_'+payload+'(tlv_msg_hdr_t *hdr, u32 tlv_buffer_size, '+function+'_t *'+payload+');'
    else:
        prototype='return_t LIBISU_decode_'+payload+'(tlv_msg_hdr_t *hdr, '+function+'_t *'+payload+');'
...
def append_define(file_ch,function,flag=""):
    ...
    with open(file_ch,'a') as f:
        if flag == 1:#we are defining encoder function
            body = '\n'+'return_t LIBISU_encode_'+payload+'(tlv_msg_hdr_t *hdr, u32 tlv_buffer_size, '+function+'_t *'+payload+')'
            body += '\n'+'{'+'\n'+'    if (LIBTLV_isu_put_opt(hdr, tlv_buffer_size, (u16)'+ID+', '+payload+', sizeof('+function+'_t),TRUE)){'+'\n'
            body += '          LIBISU_INFO_("Failed to encode '+ID+'");'+'\n'
            body += '          return RETURN_NOK;'
            body += '\n'+'}'
            body += '\n'+'    return RETURN_OK;'
            body += '\n'+'}'
        else:
            body = '\n'+'return_t LIBISU_decode_'+payload+'(tlv_msg_hdr_t *hdr,'+function+'_t *'+payload+'){'
            body += '\n'
            body += '\n'+'    u16 tmp_len = 0;'
            body += '\n'+'    void *tmp_pnt = NULL;'
            body += '\n\n'+'    if (LIBTLV_isu_get_param(hdr,(u16)'+ID+',&tmp_pnt,&tmp_len,TRUE) == 0) {'
